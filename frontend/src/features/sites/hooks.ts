...
export function useAddFocusQueueItem(siteId: string) {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (data: { priority: number; pillarUrl: string; primaryKeyword: string; secondaryKeywords: string; customTitle?: string }) =>
...
export function useImportFocusQueue(siteId: string) {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (keywords: { priority: number; pillarUrl: string; primaryKeyword: string; secondaryKeywords: string; customTitle?: string }[]) =>
