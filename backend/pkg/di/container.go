...
func NewContainer(cfg *config.Config, db *gorm.DB) *Container {
	...
	authService := serviceimpl.NewAuthService(userRepo, cfg.Google.ClientID, cfg.Google.ClientSecret, cfg.Google.RedirectURL)
	siteService := serviceimpl.NewSiteService(siteRepo, sitePageRepo, keywordRepo, aiEngine)
