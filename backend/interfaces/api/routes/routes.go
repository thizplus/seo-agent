...
func Setup(app *fiber.App, c *di.Container) {
	...
	articleHandler := handlers.NewArticleHandler(c.ArticleService, c.AIEngine, c.SiteRepo)
	...
	articles.Post("/:id/review", articleHandler.ReviewArticle)
	articles.Post("/:id/rewrite", articleHandler.RewriteArticle)
	articles.Get("/:id/page-images", articleHandler.ScrapePageImages)
	articles.Put("/:id/featured-image", articleHandler.SetFeaturedImage)
